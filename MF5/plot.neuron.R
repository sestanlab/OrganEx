...
mar_list <- lapply(names(data_list), function(ii) {
  mar_fres <- readRDS(paste0(markers, 'Mar_res.subsampled.hipo.neurons.rds')) %>%
...
plist <- lapply(names(cr_list), function(sp) {
  pp <- lapply(names(expg_list), function(dataset) {
    ...
    tt <- paste0(switch(dataset, hipo = "Neurons"), " - ",
                 switch(sp, 
  ...
  do.call(c, .)
pdf("Hipo_neuron_enrichment.pdf", width = 10, height = 4)
