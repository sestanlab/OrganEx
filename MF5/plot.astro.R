...
cr_list <- list(panr= read.table(paste0(genelist, 'panreactive_astrocytes.csv'), sep = ",", header = F)[,1])
...
mar_list <- lapply(names(data_list), function(ii) {
  mar_fres <- readRDS(paste0(markers, 'Mar_res.subsampled.hipo.astro.rds')) %>%
...
plist <- lapply(names(cr_list), function(sp) {
  pp <- lapply(names(expg_list), function(dataset) {
    ...
    tt <- paste0(switch(dataset, hipo = "Astrocytes"), " - ",
                 switch(sp, panr = "pan-reactive"))
  ...
  do.call(c, .)
pdf("Hipo_astro_enrichment.pdf", width = 4, height = 4)
