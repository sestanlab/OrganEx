...
# read in data
data_list <- list(kidney = readRDS(paste0(data, 'kidney.sampled.rds')) %>% 
...
cr_list <- list(DNArepair= read.table(paste0(genelist, 'pos_reg_of_dna_repair_GO0045739.txt'), sep = "\t", header = F)[,1],
                ...
                pct= c("LRP2","HAVCR1","KRT20","MYC","GDF15","NRG1","PVT1","PTPRM","TSHZ2","MYH9","TET3","SYNE2","B2M","LCN2", "CST3", "EGF"))

cr_list <- lapply(cr_list, function(z){ z[!is.na(z) & z != ""]})
cls_order <- list(kidney = c("h0", "h1", "h7", "ecmo", "BE"))
...
mar_list <- lapply(names(data_list), function(ii) {
  mar_fres <- readRDS(paste0(markers, 'Mar_res.subsampled.kidney.rds')) %>%
    subset(pct.1 >= 0.2 & ratio_fc >= 1.1 & avg_log2FC >= 0.2) %>%
    group_by(cluster) %>%
...
plist <- lapply(names(cr_list), function(sp) {
  pp <- lapply(names(expg_list), function(dataset) {
    ...
    tt <- paste0(switch(dataset, kidney = "Proximal Tubule"), " - ",
                 switch(sp, Apoptosis="Neg reg of apoptosis",
  ...
  do.call(c, .)
pdf("Kidney_enrichment.pdf", width = 10, height = 4)
