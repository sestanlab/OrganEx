...
# read in data
data_list <- list(heart = readRDS(paste0(data, 'heart.sampled.rds')) %>%
...
cls_order <- list(heart = c("h0", "h1", "h7", "ecmo", "BE"))
...
mar_list <- lapply(names(data_list), function(ii) {
  mar_fres <- readRDS(paste0(markers, 'Mar_res.subsampled.heart.all.rds')) %>%
    subset(ratio_fc >= 1.1 & avg_log2FC >= 0.15) %>%
    group_by(cluster) %>%
...
plist <- lapply(names(cr_list), function(sp) {
  pp <- lapply(names(expg_list), function(dataset) {
    ...
    tt <- paste0(switch(dataset, heart = "Cardiomyocytes"), " - ",
                 switch(sp,
  ...
  do.call(c, .)
pdf("Heart_enrichment.pdf", width = 8, height = 3)
plot_grid(plotlist = plist, nrow = 1, ncol = 4, align = "h")
dev.off()