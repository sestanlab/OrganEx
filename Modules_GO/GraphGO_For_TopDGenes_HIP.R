...
#        thisBEvs$cutFC2=(thisBEvs$pct.2+0.01)/(thisBEvs$pct.1+0.01);
        ...
        thisBEvs=thisBEvs[abs(thisBEvs$avg_log2FC) > 0.5,];
...
dynamicMods = cutreeDynamic(dendro = geneTree, distM = dissTOM,deepSplit = sensitivity, pamRespectsDendro = FALSE,minClusterSize = minClusterSize);new_mods <- dynamicMods
...
write.csv(mes,file="HIP_Neuron_modulesgenes.csv");
## plot network
...
write.csv(GOmoduledata,file="HIP_Neuron_GO_modules.csv");
dev.off();
