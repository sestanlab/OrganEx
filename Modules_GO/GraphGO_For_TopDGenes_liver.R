...
if (tissue=="kidney"){
   tissue_srat@meta.data$cluster[which(tissue_srat@meta.data$cluster=="Proximal Tubule")]="PRO";
   tissue_celltype=subset(tissue_srat,subset=cluster==celltype);
   }else{
   tissue_celltype=subset(tissue_srat,subset=group==celltype);
}
if (tissue=="HIP"){
   tissue_celltype=subset(tissue_celltype,subset=samplename!="p46_BE");
}
...
#        thisBEvs$cutFC2=(thisBEvs$pct.2+0.01)/(thisBEvs$pct.1+0.01);
        ...
        thisBEvs=thisBEvs[abs(thisBEvs$avg_log2FC) > 1.75,];
...
write.csv(mes,"liver_Hepatocytes_modulesgenes.csv");
## plot network
...
netmat[which(dissTOM < 0.5 & weights==1, arr.ind=T)]=1;
netmat[which(dissTOM < 0.15 & weights==0, arr.ind=T)]=1;
...
write.csv(GOmoduledata,file="liver_Hepatocytes_GO_modules.csv");
dev.off();
