...
if (TRUE){
  ...
  for (ii in names(data_list)){
    seu <- data_list[[ii]]
    all_cls  <- c("h0","h1","h7","ecmo")
    Idents(seu) <- "condition"
    marres <- lapply(all_cls, function(cls) {
      aa <- FindMarkers(seu, min.pct = 0.1, ident.1 ="BE", ident.2 = cls, logfc.threshold = 0.15, only.pos = FALSE, max.cells.per.ident = 1000) %>%
...
DEG <- list(hipo.ecmo.genes=hipo.ecmo.genes, hipo.h0.genes=hipo.h0.genes, hipo.h1.genes=hipo.h1.genes, hipo.h7.genes=hipo.h7.genes,
			heart.ecmo.genes=heart.ecmo.genes, heart.h0.genes=heart.h0.genes, heart.h1.genes=heart.h1.genes, heart.h7.genes=heart.h7.genes,
			kidney.ecmo.genes=kidney.ecmo.genes, kidney.h0.genes=kidney.h0.genes, kidney.h1.genes=kidney.h1.genes, kidney.h7.genes=kidney.h7.genes,
			liver.ecmo.genes=liver.ecmo.genes, liver.h0.genes=liver.h0.genes, liver.h1.genes=liver.h1.genes, liver.h7.genes=liver.h7.genes)

GOres = list() 
length(GOres) = length(DEG) 
names(GOres) = names(DEG) 
...
for (i in 1:length(results)){
	...
	axis.title = element_blank(),axis.text.x = element_text(size = 3),axis.ticks.length.x = unit(0.7, "mm"),
	axis.ticks.x = element_line(size = 0.4),axis.text.y = element_text(size = 5), axis.ticks.y = element_blank(), axis.line.y = element_blank())
