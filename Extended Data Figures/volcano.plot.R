...
if (TRUE){
  ...
  for (ii in names(data_list)){
    ...
    marres <- lapply(all_cls, function(cls) {
      aa <- FindMarkers(seu, min.pct = -1, ident.1 ="BE", ident.2 = cls, logfc.threshold = -1, only.pos = FALSE, max.cells.per.ident = 1000) %>%
...
for(i in 1:length(hipo)) { # loop over the list of DEGs
  results[[i]] = EnhancedVolcano(hipo[[i]],
                                 ...
                                 x = 'avg_log2FC',
                                 y = 'p_val_adj',
                                 selectLab = c(hipo.deg.pos[[i]]$gene, hipo.deg.neg[[i]]$gene),
                                 labSize = 9,
                                 ...
                                 theme(axis.line = element_line(size = 0.2), legend.position = "none",
...
for(i in 1:length(heart)) { # loop over the list of DEGs
  results[[i]] = EnhancedVolcano(heart[[i]],
                                 ...
                                 x = 'avg_log2FC',
                                 y = 'p_val_adj',
                                 selectLab = c(heart.deg.pos[[i]]$gene, heart.deg.neg[[i]]$gene),
                                 labSize = 9,
                                 col = c("grey81", "grey81", "#E1BD8E", "#CD5C5C"),
                                 pointSize = 0.1,
                                 pCutoff = 0.01,
...
for(i in 1:length(kidney)) { # loop over the list of DEGs
  results[[i]] = EnhancedVolcano(kidney[[i]],
                                 ...
                                 x = 'avg_log2FC',
                                 y = 'p_val_adj',
                                 selectLab = c(kidney.deg.pos[[i]]$gene, kidney.deg.neg[[i]]$gene),
                                 labSize = 9,
                                 col = c("grey81", "grey81", "#E1BD8E", "#CD5C5C"),
                                 pointSize = 0.1,
                                 pCutoff = 0.01,
...
for(i in 1:length(liver)) { # loop over the list of DEGs
  results[[i]] = EnhancedVolcano(liver[[i]],
                                 ...
                                 x = 'avg_log2FC',
                                 y = 'p_val_adj',
                                 selectLab = c(liver.deg.pos[[i]]$gene, liver.deg.neg[[i]]$gene),
                                 labSize = 9,
                                 col = c("grey81", "grey81", "#E1BD8E", "#CD5C5C"),
                                 pointSize = 0.1,
                                 pCutoff = 0.01,
