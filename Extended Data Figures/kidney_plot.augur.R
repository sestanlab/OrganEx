...
hip <- readRDS(file = paste0(inputdir, "OE.kidney.all.seurat.filter.final.rds"))
hip@meta.data$seurat_clusters <- as.character(hip@meta.data$seurat_clusters)


## For each pair, generate a score
all_ctps <- as.character(0:max(as.numeric(hip@meta.data$seurat_clusters)))##aug_list[[pair]][[1]]$AUC$cell_type
...
gp_colors <- c("#6650A2", "#E1BD8E", "#DFD4E9", "#FFA500") %>% 
...
pdf(paste0(outputdir, "Augur.Kidney.BOX.pdf"), width = 15, height = 10)
...
p <- ggplot(avg_data, aes_string(x = "group", y = "auc", fill = "group")) +
          ...
          theme_classic() + 
          theme(axis.line = element_line(size = 0.2), axis.ticks = element_line(size = 0.2), axis.title = element_blank(), axis.text = element_text(size = rel(0.8)), legend.position = "bottom")
pdf(paste0(outputdir, "Augur.Kidney.BOX.summarized.pdf"), width = 4, height = 4)
print(p)
dev.off()
...
jpeg(paste0(outputdir, "Augur.kidney.Seurat.UMAP.jpeg"), width = 15, height = 11, units = "in", res = 300)
plot_grid(plotlist = plist, nrow = 2, ncol = 3) %>% print()
dev.off() 
